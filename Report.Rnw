...
nonzero <- readRDS("nonzero igraph")
plot(nonzero, 
     layout=layout_in_circle,
     vertex.label.dist=0.8,
     vertex.label.degree=lab.locs,
     vertex.size=sqrt(V(nonzero)$market_value)*15/max(sqrt(V(nonzero)$market_value)),
     vertex.frame.color="NA",
     vertex.color="red",
     edge.arrow.size=1,
     edge.curved = 0.1,
     edge.lty =1,
     edge.width=0.5*E(nonzero)$weight)

legend(x=1,y=-.7, legend=1:6,lwd =0.5*(1:6) ,col = brewer.pal(6,"Greys"),y.intersp = 1,bty = "n")
